...
public class DiffTests
{
    ...
    [Test]
    public void Test_17_MultipleMinorChangesWithTrailing()
    {
        var diff = Diff.Make(@base: "ABCDEFGH", modified: "A1C2E34H");

        Assert.Multiple(
            () =>
        {
            Assert.That(diff.Blocks, Has.Count.EqualTo(7));
            ...
            Assert.That(diff.Blocks[4].Type, Is.EqualTo(BlockType.Unchanged));
            Assert.That(diff.Blocks[5].Type, Is.EqualTo(BlockType.Changed));
            Assert.That(diff.Blocks[6].Type, Is.EqualTo(BlockType.Unchanged));
        });
        ...
        Assert.That(diff.Blocks[5].OriginalLines, Is.EqualTo("FG".CharsToStringArray()));
        Assert.That(diff.Blocks[5].ModifiedLines, Is.EqualTo("34".CharsToStringArray()));
        Assert.That(diff.Blocks[6].ModifiedLines, Is.EqualTo("H".CharsToStringArray()));
    }
}
